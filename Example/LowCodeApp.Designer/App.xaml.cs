...
namespace LowCodeApp.Designer
{
    public partial class App : DesignerApp
    {
        ...
        {
            AISettings.Instance.OpenAIEndPoint =
                Environment.GetEnvironmentVariable("AZURE_OPENAI_API_ENDPOINT") ?? string.Empty;
            ...
            if (!string.IsNullOrEmpty(AISettings.Instance.OpenAIKey))
            {
                ...
                DesignerEnvironment.AddSolutionExplorerMenu(e => CreateDBInformation(e, true),
                    SolutionExplorerMenuTarget.Module, "Create DB Name (AI)", "All");
                DesignerEnvironment.AddSolutionExplorerMenu(e => CreateDBInformation(e, false),
                    SolutionExplorerMenuTarget.Module, "Create DB Name (AI)", "Empty Only");
            }
        ...
        FieldDesignBase? DbColumnTransformHandler(DataSource dataSource, DbTableDefinition table, string columnName)
        {
            ...
            if (dataSource.DataSourceType == DataSourceType.PostgreSQL && col.Name == "xmin")
                return new OptimisticLockingFieldDesign
                    { Name = SystemFieldNames.OptimisticLocking, DbColumn = columnName };
            return null;
        }
        ...
        private void ImportModulesFromDdTables()
        {
            ...
            //ユーザの選択をゲットする
            var datasourceToTables = DesignerEnvironment.GetDesignerSettings().DataSources
                .ToDictionary(e => e.Name, e => DesignerEnvironment.GetDbInfo(e.Name));
            var userSelected = DbTableSelectWindow.ShowDialog(datasourceToTables);
            if (userSelected == null) return;

            //モジュールに変換
            var err = DbTableParser.Import(DesignerEnvironment, userSelected.Value.selectedDataSource,
                userSelected.Value.selectedTables);
        ...
        private void CreateDBInformation(SolutionExplorerMenuClickEventArgs e, bool isAll)
        {
            ...
            dlg.Loaded += async (_, _) =>
            {
                ...
                var settingsFile =
                    File.ReadAllText(Path.Combine(DesignerEnvironment.CurrentFileDirectory, "designer.settings.json"));
                var clone = mod.JsonClone();
                await DbNmaeCreator.CreateDbNames(DesignerEnvironment.GetDesignerSettings(), clone, isAll);

                File.WriteAllText(Path.Combine(DesignerEnvironment.CurrentFileDirectory, "Modules", e.Item),
                    JsonConverterEx.SerializeObject(clone));
                dlg.Close();
            };
            dlg.ShowDialog();
        }
